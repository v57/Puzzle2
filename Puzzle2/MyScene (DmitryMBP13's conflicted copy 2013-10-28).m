        ...
        if(!self.completed) {
            if(self.buttonClicked) {
                ...
                if(self.puzzleNearBox) {
                    ...
                    for(Attachment*a in self.puzzleClicked.attachment) {
                        if(!a.attached && a.puzzle.onField) {
                            CGPoint pos = CGPointMake(a.puzzle.view.position.x - self.puzzleClicked.view.position.x,a.puzzle.view.position.y - self.puzzleClicked.view.position.y);
                            if(CGRectContainsPoint(a.hitbox, pos)) {
                                for(Attachment *a2 in a.puzzle.attachment) {
                                    if(a2.puzzle == self.puzzleClicked) {
                    ...
                    if(self.fieldStorage.count==self.puzzles.count || YES) {
                        int rightCount = 0;
                        for(Puzzle*puzzle in self.fieldStorage) {
                            if(puzzle != self.firstPuzzle) {
                                CGPoint rightPlace;
                                rightPlace.x = puzzle.rightPlace.origin.x+RP;
                                rightPlace.y = puzzle.rightPlace.origin.y+RP;
                                CGPoint pos;
                                pos.x = puzzle.view.position.x - self.fieldPos.x;
                                pos.y = puzzle.view.position.y - self.fieldPos.y;
                                if(CGRectContainsPoint(puzzle.rightPlace, pos)) {
                                    rightCount++;
                                }
                            }
                        }
                        if(rightCount+1 == self.puzzles.count) {
                            [self levelCompleted];
                        }
                    }
                }
...
-(void)initLevelCompleted {
    ...
    self.tip = [Button new];
    self.tip.view = [SKSpriteNode spriteNodeWithImageNamed:@"RestartButton.png"];
    self.tip.view.position = CGPointMake(TIPX,TIPY);
    self.tip.view.xScale = scale;
    self.tip.view.yScale = scale;
    self.tip.viewDown = [SKSpriteNode spriteNodeWithImageNamed:@"RestartButtonDown.png"];
...
-(void)openBox {
    ...
    for(Puzzle*puzzle in self.boxStorage) {
        ...
        SKAction *move = [SKAction moveTo:CGPointMake(150*x-70,700-150*y) duration:time];
...
-(void)showTip {
    ...
    if(!self.firstPuzzle.onField) {
        self.fieldPos = CGPointMake(300, 700);
    }
    for(Puzzle*puzzle in self.fieldStorage) {
        if(puzzle != self.firstPuzzle) {
            [puzzle.view removeAllActions];
...
-(void)clearPuzzles:(float)time {
    if(self.fieldStorage.count+self.boxStorage.count) {
        for(Puzzle*puzzle in self.fieldStorage) {
            [puzzle.view removeAllActions];
            [puzzle.view runAction:[SKAction moveBy:CGVectorMake(1200, 0) duration:time]];
        }
        for(Puzzle*puzzle in self.boxStorage) {
            [puzzle.view removeAllActions];
            [puzzle.view runAction:[SKAction moveBy:CGVectorMake(0, -50) duration:time]];
        }
...
-(void)closeCompleted:(float)time {
    if(self.completed) {
        ...
        SKAction *labelAction = [SKAction moveToY:1000 duration:time];
        self.canTouch = NO;
...
-(void)selectLevel:(int)level {
    ...
    for(int i=0;i<self.levels-1;i++) {
        if(i!=level) {
            SKAction *gtfo = [SKAction moveBy:CGVectorMake(s, 0) duration:t];
            SKSpriteNode *sprite = [self.levelPreviews objectAtIndex:i];
            [sprite runAction:gtfo];
        }
    }
